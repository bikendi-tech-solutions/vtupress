...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Successful BVN Verification</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("vp_verifications","approved","AND type = 'bvn'");
}
elseif(empty($_GET["trans_id"])){
  ...
  if(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) != 10 && strlen($_GET["trans_id"]) != 11 ){
    $id = $_GET["trans_id"];
    pagination_history_before("vp_verifications","approved","AND id = '$id' AND type = 'bvn'");
}
elseif(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) == 10 || strlen($_GET["trans_id"]) == 11 ){
  $id = $_GET["trans_id"];
  pagination_history_before("vp_verifications","approved","AND value = '$id' AND type = 'bvn'");
}
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("vp_verifications","approved","AND type = 'bvn'");
...
?>
    ...
    <td colspan="8">No BVN Verification Found</td>
    </tr>
<?php
}else{
