...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Failed Bet Funding Transactions</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("sbet","null");
}
elseif(empty($_GET["trans_id"])){
    ...
    if(is_numeric($_GET["trans_id"])){
        $id = $_GET["trans_id"];
        pagination_history_before("sbet","null","AND id = '$id'");
    }
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("sbet","null","AND request_id = '$id'");
    }
}
...
<th scope='col' class=''>Response</th>
  ...
  <select onchange="openfunction('sbet','Bet',true)">
