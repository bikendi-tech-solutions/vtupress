...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Successful Bet Funding Transactions</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("sbet","true");
}
elseif(empty($_GET["trans_id"])){
  ...
  if(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) != 10 && strlen($_GET["trans_id"]) != 11 ){
    $id = $_GET["trans_id"];
    pagination_history_before("sbet","true","AND id = '$id'");
}
elseif(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) == 10 || strlen($_GET["trans_id"]) == 11 ){
  $id = $_GET["trans_id"];
  pagination_history_before("sbet","true","AND phone = '$id'");
}
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("sbet","true","AND request_id = '$id'");
    }
}
...
foreach($transactions as $result){
    ...
    <button class='bg bg-danger' onclick='reversetransaction(\"sbet\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <i class=' fas fa-retweet'></i></button>
    </div>
    </td>
...
<th scope='col' class=''>Response</th>
  ...
  <select onchange="openfunction('sbet','Bet',false)">
