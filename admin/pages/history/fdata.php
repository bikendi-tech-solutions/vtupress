...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Pending Data Transactions</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("sdata","false");
}
elseif(empty($_GET["trans_id"])){
    pagination_history_before("sdata","false");
      ...
      if(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) != 10 && strlen($_GET["trans_id"]) != 11 ){
        $id = $_GET["trans_id"];
        pagination_history_before("sdata","false","AND id = '$id'");
    }
    elseif(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) == 10 || strlen($_GET["trans_id"]) == 11 ){
      $id = $_GET["trans_id"];
      pagination_history_before("sdata","false","AND phone = '$id'");
    }
    else{
        $id = $_GET["trans_id"];
        pagination_history_before("sdata","false","AND request_id = '$id'");
    }
}
...
foreach($transactions as $result){
    ...
    <button class='bg bg-danger' onclick='reversetransaction(\"sdata\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <i class=' fas fa-retweet bg-danger text-white'></i></button>
    <button class='bg bg-success' onclick='approvetransaction(\"sdata\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <i class='  fas fa-check bg-success text-white'></i></button>
    </div>
    </td>
...
<div class="input-group">
  ...
  <select onchange="openfunction('sdata','Data')">
