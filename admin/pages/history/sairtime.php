...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">Successful Airtime Transactions</h5>

                  <?php

if(!isset($_GET["trans_id"])){
    pagination_history_before("sairtime","true");
}
elseif(empty($_GET["trans_id"])){
  ...
  if(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) != 10 && strlen($_GET["trans_id"]) != 11 ){
    $id = $_GET["trans_id"];
    pagination_history_before("sairtime","true","AND id = '$id'");
}
elseif(is_numeric($_GET["trans_id"]) && strlen($_GET["trans_id"]) == 10 || strlen($_GET["trans_id"]) == 11 ){
  $id = $_GET["trans_id"];
  pagination_history_before("sairtime","true","AND phone = '$id'");
}
    else{
        $id = $_GET["trans_id"];
...
foreach($transactions as $result){
    ...
    <button class='bg bg-danger' onclick='reversetransaction(\"sairtime\",\"".$result->id."\", \"".$result->user_id."\", \"".$result->amount."\");'> <i class=' fas fa-retweet'></i></button>
    </div>
    </td>
