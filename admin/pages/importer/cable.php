...
<div class="card">
                <div class="card-body">
                  <h5 class="card-title">CABLE IMPORTER</h5> 
...
var obj = {};
var toatl_input = jQuery(".cable select, .cable input").length;
var run_obj;

for(run_obj = 0; run_obj <= toatl_input; run_obj++){
var current_input = jQuery(".cable select, .cable input").eq(run_obj);
...
jQuery("#cover-spin").hide();
var select = jQuery(".cable_select option:selected").text();
	swal({
  title: "Error",
  text: jQuery(".cable_select option:selected").text()+" Importer Doesn\'t Exist  For This Service",
  icon: "error",
  button: "Okay",
});	  
	  }
